...
class Graph:
    ...
    def draw(dataTable, summaryTable, y1Param, cpus, isSummary, y2param, filename):
        ...
        fig, ax1 = plt.subplots(num= y1Param + "_" + y2param, figsize=(10, 10))
        ...
        if isSummary:
            ...
            for cpu in cpus:
                ...
                ax1.plot(x, [float(data[y1Param][i][index]) for i in x] , label=cpu + " " + y1Param)
                if y2param != "None":
                    ax2 = ax1.twinx()
                    ax2.plot(x, [float(summary[y2param][i][0]) for i in x], 'r:', label=cpu + " " + y2param)
